...
const SubAccount: React.FC = () => {
  ...
  const changeStatus = async (
    ...
    if (id.indexOf('new') != -1) {
      setSubList(() => {
        subList[index].isAutoResRejection = isAutoResRejection;
        return [...subList];
      });
    } else {
      setLoading(true);
      // setSubList({ ...subList, subList[index]['isAutoResRejection']: isAutoResRejection })
      // ["isAutoResRejection"] = isAutoResRejection
      let res = await changeStatusSub(id, isAutoResRejection);
      getReject();
    }
  };
  ...
  const delPrefix = (phoneNumber: string) => {
  ...
  return (
    <Wrapper nav={<NavBar back="戻る" onBack={back} />}>
      <div className={styles.refuse}>
        <h2 className="title">子アカウント管理</h2>
        ...
        <div className="subAccountList">
          {loading ? (
            ...
            <>
                  ...
                  >
                    <div className={styles.subAccountInfo}>
                      <div className={styles.subInputBox}>
                        ...
                        <input
                          type="text"
                          placeholder={'ハイフン無し携帯番号'}
                          onChange={(e: any) =>
                            changeInput(index, e, 'phoneNumber')
                          }
                          value={delPrefix(item.phoneNumber)}
                          readOnly={item.id.indexOf('new') == -1}
                        />
                      </div>
                      {item.id.indexOf('new') == -1 ? (
                        <div className={styles.rightBox}>
                          ...
                          <div className={styles.buttonBox}>
                            ...
                            {/* <button className='unlock' onClick={(e) => handleUnlock(e, item.id)}>解锁</button> */}
                          </div>
                        </div>
                      ) : (
                        <div className={styles.rightBox}>
                    ...
                    <div className={styles.footer}>
                      ...
                      >
                        <Switch
                          size={20}
                          checked={item.isAutoResRejection}
                          onChange={(e) => changeStatus(e, item.id, index)}
                        />
                      </div>
                    </div>
                  </div>
              ...
              {/*{subList.length < 4 && (*/}
              {/*  <div*/}
              {/*    className={`${styles.subAccountBox} ${styles.addBox}`}*/}
              {/*    onClick={addSubBox}*/}
              {/*  >*/}
              {/*    <AddOutline*/}
              {/*      fontSize={48}*/}
              {/*      style={{ position: 'relative', top: '46px', left: '45%' }}*/}
              {/*    />*/}
              {/*  </div>*/}
              {/*)}*/}
            </>
